...
import { Users, Package, CheckCircle2, AlertCircle, ArrowUpRight, Heart } from "lucide-react";
...
import { AVATAR_REGISTRY_ADDRESS, avatarRegistryAbi } from "@/lib/contracts";
...
const CreatorCard = ({ creator }: { creator: Creator }) => {
  ...
  const { getSubscriberCount, recordSubscription } = useSubscriptions();
  const [modal, setModal] = useState<ModalStep>("closed");
  const [txHash, setTxHash] = useState<string | null>(null);
  const [subscribed, setSubscribed] = useState(false);
  const [isFollowing, setIsFollowing] = useState(() => {
    // Persist follow state in localStorage
    return localStorage.getItem(`following:${creator.id}`) === "true";
  });
  const [avatarUrl, setAvatarUrl] = useState<string>(creator.avatar || "");

  const subscriberCount = getSubscriberCount(creator.id);
  ...
  const confirmSubscribe = async () => {
    ...
    if (hash) {
      // Record subscription
      if (address) {
        recordSubscription(creator.id, address);
      }
      setTxHash(hash);
      setSubscribed(true);
      setModal("success");
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1.5">
              <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                <Users className="w-3 h-3" />{subscriberCount}
              </span>
            </div>
          </div>
            ...
            {subscribed ? (
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className={`text-[10px] font-bold border rounded-lg px-2 py-1 transition-all ${isFollowing
                      ? "bg-primary/20 border-primary/40 text-primary"
                      : "text-primary border-primary/30 hover:bg-primary/10"
                    }`}
                  title={isFollowing ? "Following" : "Follow"}
                >
                  <Heart className={`w-3 h-3 inline-block ${isFollowing ? 'fill-current' : ''}`} />
                </button>
                <button
              ...
              <motion.div key="confirm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="rounded-2xl bg-secondary/50 border border-white/5 divide-y divide-white/5 mb-4">
                  {[
                    ...
                    { label: "Price", value: "1 ETH", highlight: true },
