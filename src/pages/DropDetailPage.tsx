...
import { mockDrops, type Drop } from "@/lib/mockData";
import {
...
import { DROP_REGISTRY_ADDRESS, dropRegistryAbi } from "@/lib/contracts";
...
const ARTIST_REGISTRY_ABI = [
  {
    inputs: [], name: "getAllArtists", outputs: [{
      components: [
        { internalType: "address", name: "artistAddress", type: "address" },
        { internalType: "string", name: "name", type: "string" },
      ], internalType: "struct ArtistRegistry.Artist[]", name: "", type: "tuple[]"
...
const DropDetailPage = () => {
  ...
  const { isConnected, state, address, claimAsSubscriber, confirmSubscriberMint, claimPublic, reset } = useWalletAction();
  ...
  const fetchDrop = useCallback(async () => {
    ...
    try {
      const dropId = BigInt(id);

      const dropData: any = await readContract(wagmiConfig, {
      ...
      let artistName = dropData.artist.slice(0, 6) + "...";
      let creatorId = dropData.artist;
      try {
        const artists: any = await readContract(wagmiConfig, {
          address: import.meta.env.VITE_ARTIST_REGISTRY_ADDRESS as `0x${string}`,
          abi: ARTIST_REGISTRY_ABI,
          functionName: "getAllArtists",
        } as any);
        const idx = artists.findIndex((a: any) => a.artistAddress.toLowerCase() === dropData.artist.toLowerCase());
        if (idx !== -1) {
          artistName = artists[idx].name;
          creatorId = String(idx);
        }
      } catch (e) {
        // Ignore artist fetch error
      }

      setDrop({
        id: String(dropData.dropId),
        title: dropData.title,
        creator: artistName,
        creatorId: creatorId,
        creatorAvatar: "",
        image: dropData.imageHash ? getIPFSUrl(dropData.imageHash) : "",
        priceSubscriber: formatEther(dropData.priceSubscriber) + " ETH",
        pricePublic: formatEther(dropData.pricePublic) + " ETH",
        supply: Number(dropData.supply),
        claimed: Number(dropData.claimed),
        subscriberOnly: false,
        endsIn: "30d",
        hasCampaign: false,
        endTime: Number(dropData.endTime),
      });

      // Check subscription status
      if (address) {
        try {
          const subbed: any = await readContract(wagmiConfig, {
            address: DROP_REGISTRY_ADDRESS as `0x${string}`,
            abi: dropRegistryAbi,
            functionName: "isSubscribed",
            args: [address as `0x${string}`, dropData.artist as `0x${string}`],
          } as any);
          setIsSubscriber(!!subbed);
        } catch (e) {
      ...
      // Fallback
      const local = localStorage.getItem("popup:drops");
      if (local) {
        const parsed = JSON.parse(local);
        const f = parsed.find((d: Drop) => d.id === id);
        if (f) setDrop(f);
      }
    }
    setIsLoading(false);
  ...
  useEffect(() => {
    if (!drop || !drop.endTime) return;

    const calculateTimeLeft = () => {
      const now = Math.floor(Date.now() / 1000);
      const remaining = drop.endTime! - now;
  ...
  const confirmSubMint = async () => {
    if (!drop) return;
    setSubModal("pending");
    const hash = await confirmSubscriberMint(drop.priceSubscriber, drop.id);
    if (hash) { setTxHash(hash); setSubModal("success"); }
    else setSubModal("error");
  };

  const closeSubModal = () => { setSubModal("closed"); reset(); setTxHash(null); fetchDrop(); };
  ...
  const submitPub = async () => {
    ...
    const hash = await claimPublic(price, drop.id);
    if (hash) { setTxHash(hash); setPubModal("success"); }
    else setPubModal("error");
  };

  const closePubModal = () => { setPubModal("closed"); reset(); setTxHash(null); setBidAmount(""); setContentText(""); fetchDrop(); };
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
          ...
          <h1 className="text-2xl font-bold font-display mb-2">Drop Not Found</h1>
          <p className="text-muted-foreground mb-6">This drop doesn't exist or hasn't loaded properly.</p>
          <Button asChild><Link to="/drops">Back to Drops</Link></Button>
        </div>
      </Layout>
    );
  }
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <div className="mb-5 space-y-2">
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1.5 line-clamp-1 truncate max-w-[150px]"><Clock className="w-3.5 h-3.5 flex-shrink-0" />{isEnded ? "Ended" : timeLeft}</span>
              </div>
            ...
            <div className="flex gap-2 mb-4">
              <Button variant="hero" className="flex-1 rounded-xl" onClick={openSubFlow} disabled={isEnded}>
                <Shield className="w-4 h-4" /> Subscriber Claim
              </Button>
              <Button variant="neon" className="flex-1 rounded-xl" onClick={openPubFlow} disabled={isEnded}>
                Public Claim
              </Button>
            </div>
              ...
              <motion.div key="pub-choose" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <RadioGroup value={claimMethod} onValueChange={(v) => setClaimMethod(v as "funds" | "content")} className="gap-3">
                  {[
                    ...
                    <label
                      key={value}
                      className={`flex items-start gap-3 p-3.5 rounded-2xl border cursor-pointer transition-all ${claimMethod === value ? "border-primary bg-primary/8" : "border-white/8 hover:border-white/15"
                        }`}
                    >
