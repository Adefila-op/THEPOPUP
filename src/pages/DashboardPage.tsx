...
const getCreatorData = () => {
  ...
  const artists = customArtists ? JSON.parse(customArtists) : mockCreators;
  const customDrops = localStorage.getItem("popup:drops");
  const drops = customDrops ? JSON.parse(customDrops) : mockDrops;
  
  const myCreator = artists.find((c: Creator) => c.id === MY_CREATOR_ID);
  const myDrops = drops.filter((d: Drop) => d.creatorId === MY_CREATOR_ID);
  const mySubscribedCreators = artists.slice(1, 4);
  const myCollectedDrops = drops.slice(0, 2);
  const totalRevenue = myDrops.reduce((sum: number, d: Drop) => sum + parseFloat(d.priceSubscriber) * d.claimed, 0);
  
  return { myCreator, myDrops, mySubscribedCreators, myCollectedDrops, totalRevenue };
};

const { myCreator, myDrops, mySubscribedCreators, myCollectedDrops, totalRevenue } = getCreatorData();
...
const DashboardPage = () => {
  ...
  // Load creator data
  const { myCreator, myDrops, mySubscribedCreators, myCollectedDrops, totalRevenue } = useMemo(() => {
    return getCreatorData();
  }, []);
  
  const { getSubscriberCount } = useSubscriptions();
  const subscriberCount = myCreator ? getSubscriberCount(myCreator.id) : 0;
      ...
      <Layout>
        <section className="py-6">
          <div className="container mx-auto px-4 text-center py-20">
            <p className="text-muted-foreground mb-4">No creator profile found</p>
            <Link to="/">
        ...
        <div className="container mx-auto px-4 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {[
              ...
              { label: "Subscribers", value: subscriberCount.toString(), icon: Users, color: "text-blue-400" },
